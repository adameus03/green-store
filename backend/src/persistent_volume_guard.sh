...
# export SQLITE_DBS_LOCATION="/gstrpers/sqlite/databases"
# export USRFILES_LOCATION="/gstrpers/usrfiles"

echo "$SCRIPT_NICKNAME: Checking the persistent volume..."
if [ -d /gstrpers ]; then
    if [ ! -d /gstrpers/sqlite/databases ]; then
    ...
    echo "$SCRIPT_NICKNAME: ERROR - The cloud volume is not mounted at /gstrpers" 
    exit 1
fi
